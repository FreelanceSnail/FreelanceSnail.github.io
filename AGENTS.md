...
- Pages live at the root (`index.md`, `about.md`, `blog.md`, `gespenst_edge.md`, `404.html`); blog posts live in `_posts/` using Jekyll’s `YYYY-MM-DD-title.md` pattern (lowercase, hyphenated slugs). Lab pages live under `lab/` (example: `lab/momentum-rotation.md`).
- Layout templates are in `_layouts/` (`default.html`, `page.html`, `post.html`). Navigation and site meta live in `_config.yml`.
- Data files live in `_data/` (for example, `momentum_rotation.json` generated by `lab/tools/momentum_rotation.py`).
- Static assets (images, styles, downloadable files) go in `assets/` (`assets/css/style.css`, `assets/images/`); reuse `favicon.ico` unless replacing intentionally. Custom domain is set via `CNAME`.
- Local/derived artifacts (`_site/`, `vendor/`, `venv/`) should not be committed.
...
- `python lab/tools/momentum_rotation.py` — refresh lab data in `_data/momentum_rotation.json` (requires `pip install -U akshare pandas`).
